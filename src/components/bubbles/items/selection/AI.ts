    ...
    if (aiModel) {
      ...
      aiModel.chat(selectedText, prompt, {
        onStart(aiClient) {
          ...
          container.querySelector<HTMLElement>('.aie-ai-panel-body-content')!.style.display =
            'block'
      ...
      console.error(
        'AI model name config error. can not get AI model by name: "' +
          options.ai?.bubblePanelModel +
          '", please check config "options.ai.bubblePanelModel"'
      )
    }
  }
}
