...
const startChat = (holder: Holder, lang: string, textarea: HTMLTextAreaElement) => {
  if (holder.aiClient) {
    ...
    let prompt =
      holder.editor?.aiEditor.options.ai?.translate?.prompt?.(lang, selectedText) ||
        ...
        ${menuItems
          .map((menuItem) => {
            return typeof menuItem === 'string'
              ? `<p data-lang="${menuItem}">${t(menuItem)} </p>`
...
export const Translate = {
  ...
  onInit: ({ innerEditor }, tippyInstance, parentEle) => {
    ...
    const translateMenuItems =
      innerEditor.aiEditor.options.ai?.translate?.translateMenuItems || defaultTranslateMenuItems
