...
export const ImageExt = Image.extend<ImageOptions>({
  ...
  renderHTML({ HTMLAttributes }) {
    ...
    return ['div', { style: `text-align:${imgAttrs['data-align'] || 'left'}` }, ['img', imgAttrs]]
  },
...
<img alt="${alt}" src="${props.node.attrs['data-src'] || src}" style="width: ${calcImgWidth}; height: ${height || 'auto'}" class="align-${align} resize-obj">
      ...
      container.innerHTML = `
                ...
                `
      resize(container, this.editor.view.dom, (attrs) =>
        ...
        props: {
          ...
          handleDOMEvents: {
            drop(view, event) {
              ...
              dispatch(
                tr.setSelection(TextSelection.create(doc, coordinates!.pos)).scrollIntoView()
              )
