...
export const getLanguageByValueOrAlias = (valueOrAlias: string): LanguageItem | null => {
  if (!valueOrAlias) {
    return null
  }
...
export function getSelectedLineRange(selection: Selection, codeBlockNode: Node) {
    ...
    return {
      ...
      addCodeComments:
        ...
        ({ editor }) => {
          ...
          aiModel.chat(
            node.textContent,
            this.options.codeCommentsAi!.prompt,
            new DefaultAiMessageListener(editor, {
      ...
      addCodeExplain:
        ...
        ({ editor }) => {
          ...
          if (nodeSize <= pos + node.nodeSize + 2) {
            ...
            dispatch(
              tr.setSelection(TextSelection.create(editor.state.doc, pos + node.nodeSize + 1))
            )
          }
          ...
          aiModel?.chat(
            node.textContent,
            this.options.codeExplainAi!.prompt,
            new DefaultAiMessageListener(editor)
          )
          return true
        },
              ...
              return chain()
                .command(({ state, tr }) => {
                  tr.replaceRangeWith(
                    from,
                    to,
                    this.type.create(attributes, state.schema.text(textArr.join('\n')))
                  )
                  return true
                })
                .setTextSelection({
    ...
    return {
      ...
      Tab: ({ editor }) => {
        ...
        if (selection.empty) {
          ...
          const { start: selectedLineStart, end: selectedLineEnd } = getSelectedLineRange(
            selection,
            node
          )
          ...
          const newLines = lines.map((line, index) => {
            if (index >= selectedLineStart && index <= selectedLineEnd && line) {
              return tab + line
            }
            return line
          })
          ...
          tr.setSelection(
            TextSelection.between(
              ...
              tr.doc.resolve(to + (selectedLineEnd - selectedLineStart + 1) * tab.length)
            )
          )
          view.dispatch(tr)
        }
      ...
      'Shift-Tab': ({ editor }) => {
        ...
        tr.setSelection(
          TextSelection.between(tr.doc.resolve(newSelectionFrom), tr.doc.resolve(newSelectionTo))
        )
        view.dispatch(tr)
        return true
      }
  ...
  addInputRules() {
    return [
      textblockTypeInputRule({
        ...
        type: this.type,
        getAttributes: (match) => ({
          language: getLanguageByValueOrAlias(match[1])?.value || this.options.defaultLanguage
        })
      }),
      textblockTypeInputRule({
    ...
    return (props) => {
      ...
      const createEL = () => {
        ...
        div.addEventListener('click', (event) => {
          const target: HTMLDivElement = (event.target as HTMLElement).closest(
